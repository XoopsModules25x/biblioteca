...
switch ($op) 
{
    ...
    case "list":
        ...
        if ( !is_readable(XOOPS_ROOT_PATH . "/Frameworks/art/functions.admin.php"))	{
            biblioteca_adminmenu(5, _MI_biblioteca_ADMENU5);
        } else {
            include_once XOOPS_ROOT_PATH.'/Frameworks/art/functions.admin.php';
            loadModuleAdminMenu (5, _MI_biblioteca_ADMENU5);
        }
		...
		if ($numrows > 0) {
			...
			foreach (array_keys($downloadsmod_arr) as $i) {
				...
				echo '<a href="modified.php?op=view_downloads&downloads_lid=' . $downloads_lid . '&mod_id=' . $downloads_requestid . '"><img src="../images/icon/view_mini.png" alt="' . _AM_biblioteca_FORMDISPLAY . '" title="' . _AM_biblioteca_FORMDISPLAY . '"></a> ';
                echo '<a href="modified.php?op=del_moddownloads&mod_id=' . $downloads_requestid . '&new_file=' . $new_file . '"><img src="../images/icon/ignore_mini.png" alt="' . _AM_biblioteca_FORMIGNORE . '" title="' . _AM_biblioteca_FORMIGNORE . '"></a>';
				echo '</td>';
			 }
			 echo '</table><br />';
			 echo '<br /><div align=right>' . $pagenav . '</div><br />';
		}else{
            echo '<div class="errorMsg" style="text-align: center;">' . _AM_biblioteca_ERREUR_NOBMODDOWNLOADS . '</div>';
        }
	break; 
    ...
    case "view_downloads":
        ...
        echo '<h4>' . _AM_biblioteca_MODIFIED_ORIGINAL . '</h4>';
        echo '<table width="100%">';
        echo '<tr>';
        echo '<td valign="top" width="50%"><small><span class="' . ($downloads_title == $moddownloads_title ? 'style_ide' : 'style_dif') . '">' . _AM_biblioteca_FORMTITLE . '</span>: ' . $downloads_title . '</small></td>';
        echo '<td valign="top" rowspan="14"><small><span class="' . ($downloads_description == $moddownloads_description ? 'style_ide' : 'style_dif') . '">' . _AM_biblioteca_FORMTEXT . '</span>:<br />' . $downloads_description . '</small></td>';
        echo '</tr>';
        echo '<tr><td valign="top" width="40%"><small><span class="' . ($downloads_url == $moddownloads_url ? 'style_ide' : 'style_dif') . '">' . _AM_biblioteca_FORMURL . '</span>:<br />' . $downloads_url . '</small></td></tr>';
        echo '<tr><td valign="top" width="40%"><small><span class="' . ($downloads_categorie == $moddownloads_categorie ? 'style_ide' : 'style_dif') . '">' . _AM_biblioteca_FORMCAT . '</span>: ' . $downloads_categorie . '</small></td></tr>';
        $criteria = new CriteriaCompo();
        ...
        foreach (array_keys($downloads_field) as $i) {
            if ($downloads_field[$i]->getVar('status_def') == 1){
                if ($downloads_field[$i]->getVar('fid') == 1){
                    //page d'accueil
                    echo '<tr><td valign="top" width="40%"><small><span class="' . ($downloads_homepage == $moddownloads_homepage ? 'style_ide' : 'style_dif') . '">' . _AM_biblioteca_FORMHOMEPAGE . '</span>: <a href="' . $downloads_homepage . '">' . $downloads_homepage . '</a></small></td></tr>';
                }
                if ($downloads_field[$i]->getVar('fid') == 2){
                    //version
                    echo '<tr><td valign="top" width="40%"><small><span class="' . ($downloads_version == $moddownloads_version ? 'style_ide' : 'style_dif') . '">' . _AM_biblioteca_FORMVERSION . '</span>: ' . $downloads_version . '</small></td></tr>';
                }
                if ($downloads_field[$i]->getVar('fid') == 3){
                    //taille du fichier
                    echo '<tr><td valign="top" width="40%"><small><span class="' . ($downloads_size == $moddownloads_size ? 'style_ide' : 'style_dif') . '">' . _AM_biblioteca_FORMSIZE . '</span>: ' . $downloads_size  . '</small></td></tr>';
                }
                if ($downloads_field[$i]->getVar('fid') == 4){
                    //plateforme
                    echo '<tr><td valign="top" width="40%"><small><span class="' . ($downloads_platform == $moddownloads_platform ? 'style_ide' : 'style_dif') . '">' . _AM_biblioteca_FORMPLATFORM . '</span>: ' . $downloads_platform  . '</small></td></tr>';
                }
            }else{
                //original
                $contenu = '';
        ...
        echo '<tr><td valign="top" width="40%"><small><span class="' . ($downloads_logourl == $moddownloads_logourl ? 'style_ide' : 'style_dif') . '">' . _AM_biblioteca_FORMIMG . '</span>:<br /> <img src="' . $uploadurl_shots . $downloads_logourl . '" alt="" title=""></small></td></tr>';
        echo '</table>';
        //proposé
        echo '</td></tr><tr><td>';
        echo '<h4>' . _AM_biblioteca_MODIFIED_MOD . '</h4>';
        echo '<table width="100%">';
        echo '<tr>';
        echo '<td valign="top" width="40%"><small><span class="' . ($downloads_title == $moddownloads_title ? 'style_ide' : 'style_dif') . '">' . _AM_biblioteca_FORMTITLE . '</span>: ' . $moddownloads_title . '</small></td>';
        echo '<td valign="top" rowspan="14"><small><span class="' . ($downloads_description == $moddownloads_description ? 'style_ide' : 'style_dif') . '">' . _AM_biblioteca_FORMTEXT . '</span>:<br />' . $moddownloads_description . '</small></td>';
        echo '</tr>';
        echo '<tr><td valign="top" width="40%"><small><span class="' . ($downloads_url == $moddownloads_url ? 'style_ide' : 'style_dif') . '">' . _AM_biblioteca_FORMURL . '</span>:<br />' . $moddownloads_url . '</small></td></tr>';
        echo '<tr><td valign="top" width="40%"><small><span class="' . ($downloads_categorie == $moddownloads_categorie ? 'style_ide' : 'style_dif') . '">' . _AM_biblioteca_FORMCAT . '</span>: ' . $moddownloads_categorie . '</small></td></tr>';
        $criteria = new CriteriaCompo();
        $criteria->setSort('weight ASC, title');
        $criteria->setOrder('ASC');
        $criteria->add(new Criteria('status', 1));
        ...
        foreach (array_keys($downloads_field) as $i) {
            if ($downloads_field[$i]->getVar('status_def') == 1){
                if ($downloads_field[$i]->getVar('fid') == 1){
                    //page d'accueil
                    echo '<tr><td valign="top" width="40%"><small><span class="' . ($downloads_homepage == $moddownloads_homepage ? 'style_ide' : 'style_dif') . '">' . _AM_biblioteca_FORMHOMEPAGE . '</span>: <a href="' . $moddownloads_homepage . '">' . $moddownloads_homepage . '</a></small></td></tr>';
                }
                if ($downloads_field[$i]->getVar('fid') == 2){
                    //version
                    echo '<tr><td valign="top" width="40%"><small><span class="' . ($downloads_version == $moddownloads_version ? 'style_ide' : 'style_dif') . '">' . _AM_biblioteca_FORMVERSION . '</span>: ' . $moddownloads_version . '</small></td></tr>';
                }
                if ($downloads_field[$i]->getVar('fid') == 3){
                    //taille du fichier
                    echo '<tr><td valign="top" width="40%"><small><span class="' . ($downloads_size == $moddownloads_size ? 'style_ide' : 'style_dif') . '">' . _AM_biblioteca_FORMSIZE . '</span>: ' . $moddownloads_size  . '</small></td></tr>';
                }
                if ($downloads_field[$i]->getVar('fid') == 4){
                    //plateforme
                    echo '<tr><td valign="top" width="40%"><small><span class="' . ($downloads_platform == $moddownloads_platform ? 'style_ide' : 'style_dif') . '">' . _AM_biblioteca_FORMPLATFORM . '</span>: ' . $moddownloads_platform  . '</small></td></tr>';
                }
            }else{
                //original
                $contenu = '';
        ...
        echo '<tr><td valign="top" width="40%"><small><span class="' . ($downloads_logourl == $moddownloads_logourl ? 'style_ide' : 'style_dif') . '">' . _AM_biblioteca_FORMIMG . '</span>:<br /> <img src="' . $uploadurl_shots . $moddownloads_logourl . '" alt="" title=""></small></td></tr>';
        echo '</table>';
        echo '</table>';
        echo '</td></tr></table>';
        ...
        $new_file = ($downloads_url == $moddownloads_url ? false : true);
        echo '<table><tr><td>';
        echo myTextForm('modified.php?op=approve&mod_id=' . $_REQUEST['mod_id'] . '&new_file=' . $new_file , _AM_biblioteca_FORMAPPROVE);
        echo '</td><td>';
        echo myTextForm('downloads.php?op=edit_downloads&downloads_lid=' . $_REQUEST['downloads_lid'], _AM_biblioteca_FORMEDIT);
        echo '</td><td>';
        echo myTextForm('modified.php?op=del_moddownloads&mod_id=' . $_REQUEST['mod_id'] . '&new_file=' . $new_file, _AM_biblioteca_FORMIGNORE);
        echo '</td></tr></table>';
    break;
    ...
    case "del_moddownloads":
        ...
        if (isset($_REQUEST['ok']) && $_REQUEST['ok'] == 1) {
            ...
            if ($downloadsmod_Handler->delete($obj)) {
                redirect_header('modified.php', 1, _AM_biblioteca_REDIRECT_DELOK);
            }
			...
			xoops_confirm(array('ok' => 1, 'mod_id' => $_REQUEST['mod_id'], 'new_file' => $_REQUEST['new_file'], 'op' => 'del_moddownloads'), $_SERVER['REQUEST_URI'], _AM_biblioteca_MODIFIED_SURDEL . '<br>');
		}
    break;
    ...
    case "approve":
        ...
        if ($downloads_Handler->insert($obj)){
            redirect_header('modified.php', 1, _AM_biblioteca_REDIRECT_SAVE);
        }

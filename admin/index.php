...
function moduleLastVersionInfo($version, $module_name)
{
    ...
    if (ini_get('allow_url_fopen')) {
        ...
        if (!$fp) {
            ...
            fclose($fp);
            if (preg_match('/<module>(.*)</module>/i', $raw, $rawelements)) {
                $elements = explode('<module>', $rawelements[0]);
                $maximum  = count($elements);
                for ($i = 1; $i < $maximum; $i++) {
                    preg_match('/<name>(.*)</name>/i', $elements[$i], $name);
                    preg_match('/<version>(.*)</version>/i', $elements[$i], $version);
                    preg_match('/<xoopsVersionNeeded>(.*)</xoopsVersionNeeded>/i', $elements[$i], $xoopsVersionNeeded);
                    preg_match('/<versionChangelog>(.*)</versionChangelog>/i', $elements[$i], $versionChangelog);

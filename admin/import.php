...
function Import_mydownloads($path = '', $imgurl = '')
{
    ...
    if ($ok == 1) {
        ...
        while ($donnees = $xoopsDB->fetchArray($query_links)) {
			...
			('" . $donnees['lid'] . "', '" . $donnees['cid'] . "', '" . $donnees['title'] . "', '" . $donnees['url'] . "', '" . $donnees['homepage'] . "', '" . $donnees['version'] . "', '" . $donnees['size'] . "', '" . $donnees['platform'] . "', '" . $description . "',  '" . $donnees['logourl']
                                       . "', '" . $donnees['submitter'] . "', '" . $donnees['status'] . "', '" . $donnees['date'] . "', '" . $donnees['hits'] . "', '" . $donnees['rating'] . "', '" . $donnees['votes'] . "', '0', '0' )");
            if (!$insert) {
                echo "<font color='red'>" . _AM_BIBLIOTECA_IMPORT_ERROR_DATA . ': </font> ' . $donnees['title'] . '<br>';
            }
        ...
        while ($donnees = $xoopsDB->fetchArray($query_vote)) {
            $insert =
                $xoopsDB->queryF('INSERT INTO ' . $xoopsDB->prefix('biblioteca_votedata') . " (ratingid, lid, ratinguser, rating, ratinghostname, ratingtimestamp ) VALUES ('" . $donnees['ratingid'] . "', '" . $donnees['lid'] . "', '" . $donnees['ratinguser'] . "', '" . $donnees['rating'] . "', '"
                                 . $donnees['ratinghostname'] . "', '" . $donnees['ratingtimestamp'] . "')");
            if (!$insert) {
...
function Import_wfdownloads($shots = '', $catimg = '')
{
    ...
    if ($ok == 1) {
        ...
        while ($donnees = $xoopsDB->fetchArray($query_topic)) {
            ...
            $insert = $xoopsDB->queryF('INSERT INTO ' . $xoopsDB->prefix('biblioteca_cat') . " (cat_cid, cat_pid, cat_title, cat_imgurl, cat_description_main, cat_weight ) VALUES ('" . $donnees['cid'] . "', '" . $donnees['pid'] . "', '" . $donnees['title'] . "', '" . $img . "', '"
                                       . addcslashes($donnees['description'], "'") . "', '" . $donnees['weight'] . "')");
            if (!$insert) {
                echo "<font color='red'>" . _AM_BIBLIOTECA_IMPORT_ERROR_DATA . ': </font> ' . $donnees['title'] . '<br>';
            }
            echo sprintf(_AM_BIBLIOTECA_IMPORT_CAT_IMP . '<br>', $donnees['title']);

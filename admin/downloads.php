...
$statut_menu = biblioteca_CleanVars($_REQUEST, 'statut_display', 1, 'int');
...
switch ($op) 
{
    ...
    case "list":
        ...
        if ( !is_readable(XOOPS_ROOT_PATH . "/Frameworks/art/functions.admin.php"))	{
            biblioteca_adminmenu(3, _MI_biblioteca_ADMENU3);
        } else {
            include_once XOOPS_ROOT_PATH.'/Frameworks/art/functions.admin.php';
            loadModuleAdminMenu (3, _MI_biblioteca_ADMENU3);
        }
        // Sous-menu
        echo '<div class="head" align="center">';
        echo $op == 'new_downloads' ? _AM_biblioteca_DOWNLOADS_NEW : '<a href="downloads.php?op=new_downloads">' . _AM_biblioteca_DOWNLOADS_NEW . '</a>';
        echo ' | ';
        echo $op == 'list' && $statut_menu == 1 ? _AM_biblioteca_DOWNLOADS_LISTE : '<a href="downloads.php?op=list">' . _AM_biblioteca_DOWNLOADS_LISTE . '</a>';
        echo ' | ';
        echo $op == 'list' && $statut_menu == 0 ? _AM_biblioteca_DOWNLOADS_WAIT . ($downloads_waiting == 0 ? '' : ' (<span style="color : Red">' . $downloads_waiting . '</span>)') : '<a href="downloads.php?op=list&statut_display=0">' . _AM_biblioteca_DOWNLOADS_WAIT . ($downloads_waiting == 0 ? '' : ' (<span style="color : Red">' . $downloads_waiting . '</span>)').'</a>';
        echo '</div><br>';
		...
		if ($numrowscat==0) {
			redirect_header('category.php?op=new_cat', 2, _AM_biblioteca_REDIRECT_NOCAT);
		}
        $criteria = new CriteriaCompo();
        ...
        $start = biblioteca_CleanVars ( $_REQUEST, 'start', 0, 'int' );
		...
		if ($numrows>0) {
			...
			echo _AM_biblioteca_TRIPAR . "<select name=\"document_tri\" id=\"document_tri\" onchange=\"location='".XOOPS_URL."/modules/".$xoopsModule->dirname()."/admin/downloads.php?statut_display=$statut_display&document_order=$document_order&document_tri='+this.options[this.selectedIndex].value\">";
			echo '<option value="1"' . ($document_tri == 1 ? ' selected="selected"' : '') . '>' . _AM_biblioteca_FORMDATE . '</option>';
			echo '<option value="2"' . ($document_tri == 2 ? ' selected="selected"' : '') . '>' . _AM_biblioteca_FORMTITLE . '</option>';
			echo '<option value="3"' . ($document_tri == 3 ? ' selected="selected"' : '') . '>' . _AM_biblioteca_FORMHITS . '</option>';
            echo '<option value="4"' . ($document_tri == 4 ? ' selected="selected"' : '') . '>' . _AM_biblioteca_FORMRATING . '</option>';
            echo '<option value="5"' . ($document_tri == 5 ? ' selected="selected"' : '') . '>' . _AM_biblioteca_FORMCAT . '</option>';
			echo '</select> ';
			echo _AM_biblioteca_ORDER . "<select name=\"order_tri\" id=\"order_tri\" onchange=\"location='".XOOPS_URL."/modules/".$xoopsModule->dirname()."/admin/downloads.php?statut_display=$statut_display&document_tri=$document_tri&document_order='+this.options[this.selectedIndex].value\">";
			...
			echo '<tr>';
			echo '<th align="center" width="5%">'._AM_biblioteca_FORMFILE.'</th>';
			echo '<th align="left" width="20%">' . _AM_biblioteca_FORMTITLE . '</th>';
			echo '<th align="left">' . _AM_biblioteca_FORMCAT . '</th>';
            echo '<th align="center" width="5%">' . _AM_biblioteca_FORMHITS . '</th>';
            echo '<th align="center" width="5%">' . _AM_biblioteca_FORMRATING . '</th>';
			...
			foreach (array_keys($downloads_arr) as $i) {
                ...
                $category = biblioteca_PathTree($mytree, $downloads_arr[$i]->getVar('cid'), $category_arr, 'cat_title', $prefix = ' <img src="../images/deco/arrow.gif"> ' );
 				echo '<tr class="'.$class.'">';
				...
				echo '<td align="center">';
				echo ($statut_display == 1 ? '<a href="downloads.php?op=lock_status&downloads_lid=' . $i . '"><img src="./../images/icon/lock.png" border="0" alt="' . _AM_biblioteca_FORMLOCK . '" title="' . _AM_biblioteca_FORMLOCK . '"></a> ' : '<a href="downloads.php?op=update_status&downloads_lid=' . $i . '"><img src="./../images/icon/renew_mini.gif" border="0" alt="' . _AM_biblioteca_FORMVALID . '" title="' . _AM_biblioteca_FORMVALID . '"></a> ');
				echo '<a href="downloads.php?op=view_downloads&downloads_lid=' . $i . '"><img src="../images/icon/view_mini.png" alt="' . _AM_biblioteca_FORMDISPLAY.'" title="' . _AM_biblioteca_FORMDISPLAY . '"></a> ';
                echo '<a href="downloads.php?op=edit_downloads&downloads_lid=' . $i . '"><img src="../images/icon/edit_mini.gif" alt="' . _AM_biblioteca_FORMEDIT.'" title="' . _AM_biblioteca_FORMEDIT . '"></a> ';
                echo '<a href="downloads.php?op=del_downloads&downloads_lid=' . $i . '"><img src="../images/icon/delete_mini.gif" alt="' . _AM_biblioteca_FORMDEL.'" title="' . _AM_biblioteca_FORMDEL . '"></a>';
				echo '</td>';
			 }
			 echo '</table><br />';
			 echo '<br /><div align=right>' . $pagenav . '</div><br />';
		}else{
            echo '<div class="errorMsg" style="text-align: center;">' . _AM_biblioteca_ERREUR_NODOWNLOADS . '</div>';
        }
    ...
    case "edit_downloads":
        ...
        echo '</div><br>';
        //Affichage du formulaire de création des téléchargements
        $downloads_lid = biblioteca_CleanVars($_REQUEST, 'downloads_lid', 0, 'int');
    ...
    case "del_downloads":
		...
		if (isset($_REQUEST['ok']) && $_REQUEST['ok'] == 1) {
			...
			if ($downloads_Handler->delete($obj)) {
				...
				redirect_header('downloads.php', 1, _AM_biblioteca_REDIRECT_DELOK);
			} else {
            ...
            echo $op == 'list' && $statut_menu == 0 ? _AM_biblioteca_DOWNLOADS_WAIT . ($downloads_waiting == 0 ? '' : ' (<span style="color : Red">' . $downloads_waiting . '</span>)') : '<a href="downloads.php?op=list&statut_display=0">' . _AM_biblioteca_DOWNLOADS_WAIT . ($downloads_waiting == 0 ? '' : ' (<span style="color : Red">' . $downloads_waiting . '</span>)').'</a>';
            echo '</div><br>';
			xoops_confirm(array('ok' => 1, 'downloads_lid' => $downloads_lid, 'op' => 'del_downloads'), $_SERVER['REQUEST_URI'], sprintf(_AM_biblioteca_FORMSUREDEL, $obj->getVar('title')) . '<br><br>' . _AM_biblioteca_FORMWITHFILE . ' <b><a href="' . $obj->getVar('url') . '">' . $obj->getVar('url') . '</a></b><br>');
		}
    break;
    ...
    case "view_downloads":
        ...
        $category = biblioteca_PathTree($mytree, $view_downloads->getVar('cid'), $category_arr, 'cat_title', $prefix = ' <img src="../images/deco/arrow.gif"> ' );
		...
		echo '</tr>';
        echo '<tr class="even">';
		echo '<td width="30%">' . _AM_biblioteca_FORMFILE . ' </td>';
		...
		echo '</tr>';
        echo '<tr class="odd">';
		echo '<td width="30%">' . _AM_biblioteca_FORMCAT . ' </td>';
		echo '<td>' . $category . '</td>';
		echo '</tr>';
        $criteria = new CriteriaCompo();
            ...
            if ($downloads_field[$i]->getVar('status_def') == 1){
                if ($downloads_field[$i]->getVar('fid') == 1){
                    ...
                    if ($view_downloads->getVar('homepage') !=''){
                        $class = ($class == 'even') ? 'odd' : 'even';
                        echo '<tr class="' . $class . '">';
                        echo '<td width="30%">' . _AM_biblioteca_FORMHOMEPAGE . ' </td>';
                ...
                if ($downloads_field[$i]->getVar('fid') == 2){
                    ...
                    if ($view_downloads->getVar('version') !=''){
                        $class = ($class == 'even') ? 'odd' : 'even';
                        echo '<tr class="' . $class . '">';
                        echo '<td width="30%">' . _AM_biblioteca_FORMVERSION . ' </td>';
                ...
                if ($downloads_field[$i]->getVar('fid') == 3){
                    ...
                    if ($view_downloads->getVar('size') !=''){
                        $class = ($class == 'even') ? 'odd' : 'even';
                        echo '<tr class="' . $class . '">';
                        echo '<td width="30%">' . _AM_biblioteca_FORMSIZE . '</td>';
                ...
                if ($downloads_field[$i]->getVar('fid') == 4){
                    ...
                    if ($view_downloads->getVar('platform') !=''){
                        $class = ($class == 'even') ? 'odd' : 'even';
                        echo '<tr class="' . $class . '">';
                        echo '<td width="30%">' . _AM_biblioteca_FORMPLATFORM . ' </td>';
        ...
        $class = ($class == 'even') ? 'odd' : 'even';
		echo '<tr class="' . $class . '">';
		echo '<td width="30%">' . _AM_biblioteca_FORMTEXT . ' </td>';
        ...
        if ( $xoopsModuleConfig['useshots']){
            if ($view_downloads->getVar('logourl') != 'blank.gif'){
                $class = ($class == 'even') ? 'odd' : 'even';
                echo '<tr class="' . $class . '">';
                echo '<td width="30%">' . _AM_biblioteca_FORMIMG . ' </td>';
		...
		$class = ($class == 'even') ? 'odd' : 'even';
		echo '<tr class="' . $class . '">';
		echo '<td width="30%">' . _AM_biblioteca_FORMDATE . ' </td>';
		echo '<td>' . formatTimestamp($view_downloads->getVar('date')) . '</td>';
		echo '</tr>';        
		$class = ($class == 'even') ? 'odd' : 'even';
		echo '<tr class="' . $class . '">';
		echo '<td width="30%">' . _AM_biblioteca_FORMPOSTER . ' </td>';
		echo '<td>' . XoopsUser::getUnameFromId($view_downloads->getVar('submitter')) . '</td>';
		echo '</tr>';        
		$class = ($class == 'even') ? 'odd' : 'even';
		echo '<tr class="' . $class . '">';
		echo '<td width="30%">' . _AM_biblioteca_FORMHITS . ' </td>';
		echo '<td>' . $view_downloads->getVar('hits') . '</td>';
		echo '</tr>';
		$class = ($class == 'even') ? 'odd' : 'even';
		echo '<tr class="' . $class . '">';
		echo '<td width="30%">' . _AM_biblioteca_FORMRATING . ' </td>';
		echo '<td>' . number_format($view_downloads->getVar('rating'),1) . ' (' . $view_downloads->getVar('votes') . ' ' . _AM_biblioteca_FORMVOTE . ')</td>';
		echo '</tr>';
        if( $view_downloads->getVar('paypal') != '' && $xoopsModuleConfig['use_paypal'] == true) {
            $class = ($class == 'even') ? 'odd' : 'even';
            echo '<tr class="' . $class . '">';
            echo '<td width="30%">' . _AM_biblioteca_FORMPAYPAL . ' </td>';
            echo '<td>' . $view_downloads->getVar('paypal') . '</td>';
            echo '</tr>';
        }
		$class = ($class == 'even') ? 'odd' : 'even';
		echo '<tr class="' . $class . '">';
		echo '<td width="30%">' . _AM_biblioteca_FORMCOMMENTS . ' </td>';
		echo '<td>' . $view_downloads->getVar('comments') . ' <a href="../singlefile.php?cid=' . $view_downloads->getVar('cid') . '&lid=' . $downloads_lid . '"><img src="../images/icon/view_mini.png" alt="' . _AM_biblioteca_FORMDISPLAY . '" title="' . _AM_biblioteca_FORMDISPLAY . '"></a></td>';
		echo '</tr>';
		$class = ($class == 'even') ? 'odd' : 'even';
		echo '<tr class="' . $class . '">';
		echo '<td width="30%">' . _AM_biblioteca_FORMACTION . ' </td>';
		echo '<td>';
        echo ($view_downloads->getVar('status') != 0 ? '' : '<a href="downloads.php?op=update_status&downloads_lid=' . $downloads_lid . '"><img src="./../images/icon/renew_mini.gif" border="0" alt="' . _AM_biblioteca_FORMVALID . '" title="' . _AM_biblioteca_FORMVALID . '"></a> ');
        echo '<a href="downloads.php?op=edit_downloads&downloads_lid=' . $downloads_lid . '"><img src="../images/icon/edit_mini.gif" alt="' . _AM_biblioteca_FORMEDIT .
        '" title="' . _AM_biblioteca_FORMEDIT . '"></a> <a href="downloads.php?op=del_downloads&downloads_lid=' . $downloads_lid . '">
        <img src="../images/icon/delete_mini.gif" alt="' . _AM_biblioteca_FORMDEL . '" title="' . _AM_biblioteca_FORMDEL . '"></a></td>';
		echo '</tr>';
		echo '</table>';
		echo '<br>';        
        // Affichage des votes:
        ...
        printf(_AM_biblioteca_DOWNLOADS_VOTESUSER, $total_vote);
        echo '</b><br /><br /></td></tr>';
        echo '<tr><td><b>' . _AM_biblioteca_DOWNLOADS_VOTE_USER . '</b></td>' . '<td><b>' . _AM_biblioteca_DOWNLOADS_VOTE_IP . '</b></td>' . '<td align="center"><b>' . _AM_biblioteca_FORMRATING . '</b></td>'
        . '<td><b>' . _AM_biblioteca_FORMDATE . '</b></td>' . '<td align="center"><b>' . _AM_biblioteca_FORMDEL . '</b></td></tr>';
        ...
        printf(_AM_biblioteca_DOWNLOADS_VOTESANONYME, $total_vote);
        echo '</b><br /><br /></td></tr>';   
        echo '<tr><td colspan="2"><b>' . _AM_biblioteca_DOWNLOADS_VOTE_IP . '</b></td>' . '<td align="center"><b>' . _AM_biblioteca_FORMRATING . '</b></td>'
        . '<td><b>' . _AM_biblioteca_FORMDATE . '</b></td>' . '<td align="center"><b>' . _AM_biblioteca_FORMDEL . '</b></td></tr>';
        foreach (array_keys($downloadsvotedata_arr) as $i) {
            echo '<tr>';
        ...
        if ($downloadsvotedata_Handler->delete($objvotedata)) {
            ...
            if ($total_vote == 0){
                ...
                if ($downloads_Handler->insert($obj)) {
                    redirect_header('downloads.php?op=view_downloads&downloads_lid=' . $_REQUEST['lid'], 1, _AM_biblioteca_REDIRECT_DELOK);
                }
            }else{
                $total_rating = 0;
                foreach (array_keys($downloadsvotedata_arr) as $i) {
	...
	case "save_downloads":
        ...
        if (intval($_REQUEST['size']) == 0){
            if ($_REQUEST['size'] == '0' || $_REQUEST['size'] == ''){
                ...
                $erreur = true;
                $message_erreur .= _AM_biblioteca_ERREUR_SIZE . '<br>';
            }
        }
        ...
        if (isset($_REQUEST['description'])){
            if ($_REQUEST['description'] == ''){
                $erreur=true;
                $message_erreur .= _AM_biblioteca_ERREUR_NODESCRIPTION . '<br>';
            }
        }
        ...
        if (isset($_REQUEST['cid'])){
            if ($_REQUEST['cid'] == 0){
                $erreur = true;
                $message_erreur .= _AM_biblioteca_ERREUR_NOCAT . '<br>';
            }
        }
        ...
        if ($erreur == 1){
            ...
            if ($downloads_Handler->insert($obj)) {
                ...
                if ($xoopsModuleConfig['permission_download'] == 1) {
                    ...
                    if(isset($_POST['item_download'])) {
                        foreach($_POST['item_download'] as $onegroup_id) {
                            $gperm_handler->addRight('biblioteca_download_item', $lid_dowwnloads, $onegroup_id, $xoopsModule->getVar('mid'));
                        }
                    }
                }
                ...
                redirect_header('downloads.php', 2, _AM_biblioteca_REDIRECT_SAVE);
            }
            echo $obj->getHtmlErrors();
        }
    ...
    case "update_status":
        ...
        $obj->setVar('status', 1);
        if ($downloads_Handler->insert($obj)) {
            redirect_header('downloads.php?op=list&statut_display=0', 1, _AM_biblioteca_REDIRECT_SAVE);
        }        
        echo $obj->getHtmlErrors();
    break;

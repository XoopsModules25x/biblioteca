...
if (!in_array((int)$_REQUEST['cid'], $categories)) {
...
if ($xoopsModuleConfig['perpage'] > 0) {
    ...
    if ($numrows > 1) {
        ...
        $xoopsTpl->assign('category_id', (int)$_REQUEST['cid']);
        $sortorder = $sort . $order;
        if ($sortorder === 'hitsASC') {
            $affichage_tri = _MD_BIBLIOTECA_CAT_POPULARITYLTOM;
        }
        if ($sortorder === 'hitsDESC') {
            $affichage_tri = _MD_BIBLIOTECA_CAT_POPULARITYMTOL;
        }
        if ($sortorder === 'titleASC') {
            $affichage_tri = _MD_BIBLIOTECA_CAT_TITLEATOZ;
        }
        if ($sortorder === 'titleDESC') {
            $affichage_tri = _MD_BIBLIOTECA_CAT_TITLEZTOA;
        }
        if ($sortorder === 'dateASC') {
            $affichage_tri = _MD_BIBLIOTECA_CAT_DATEOLD;
        }
        if ($sortorder === 'dateDESC') {
            $affichage_tri = _MD_BIBLIOTECA_CAT_DATENEW;
        }
        if ($sortorder === 'ratingASC') {
            $affichage_tri = _MD_BIBLIOTECA_CAT_RATINGLTOH;
        }
        if ($sortorder === 'ratingDESC') {
            $affichage_tri = _MD_BIBLIOTECA_CAT_RATINGHTOL;
        }
